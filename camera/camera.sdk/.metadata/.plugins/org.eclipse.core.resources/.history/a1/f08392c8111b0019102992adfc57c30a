...
int jo_write_jpg( const void *data, int width, int height, int comp, int quality)
{
...
int TakeJPG()
{
	...
	FRESULT status;
	SD_Pic= (char *)pic;
	status = f_open(&f, SD_Pic, FA_OPEN_ALWAYS | FA_WRITE | FA_READ);
	if(status != FR_OK){
		return XST_FAILURE;
	}
